...
class FunSetSuite extends FunSuite {
  ...
  test("intersects contains common elements") {
    new TestSets {
  ...
  test("diff returns set difference") {
    new TestSets {
      val s4 = union( union(s1, s2), union(s2, s3) )
      val s = diff(s4, s3)
  ...
  test("filter returns elements that satisfy predicate") {
    new TestSets {

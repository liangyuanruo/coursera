...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % Begin backprop
  error = class_output - data.targets;

  % Compute the delta error for the hidden layer
  delta_hidden = (model.hid_to_class' * error) .* (logistic(hid_input) .* (1 - logistic(hid_input)));

  % Normalised gradient for output layer across all training examples + weight decay
  ret.hid_to_class = (1/m) .* (error * hid_output') + wd_coefficient .* model.hid_to_class;

  % Normalised gradient for hidden layer across all training examples + weight decay

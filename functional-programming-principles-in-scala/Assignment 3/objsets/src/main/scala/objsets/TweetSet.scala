...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      ...
      val accTotal = right.filterAcc(p, left.filterAcc(p, acc))

      if(p(elem)) accTotal.incl(elem)
      else accTotal
    }
    ...
    def mostRetweetedAcc(acc: Tweet): Tweet = right.mostRetweetedAcc( left.mostRetweetedAcc(
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter( t => google.exists( x => t.text.contains(x) ) )

  lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter( t => apple.exists( x => t.text.contains(x) ) )
